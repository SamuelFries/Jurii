...
Widget _host(void Function(AppointmentDraft?) onResult, {Appointment? existing}) {
  return MaterialApp(
      ...
      builder: (context) => Scaffold(
        body: Center(
          child: ElevatedButton(
            onPressed: () async {
              onResult(
                await showAppointmentFormSheet(context, existing: existing),
              );
            },
...
void main() {
  group('Appointment.isEditable', () {
    test('editável só com horários reais (linha do backend)', () {
      ...
      expect(
        _appointment(startsAt: now, endsAt: now.add(const Duration(hours: 1)))
            .isEditable,
        isTrue,
      );
    });
